...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if (isset($requestdata['customer_email']) && count($requestdata['customer_email']) && $requestdata['customer_email'] != '') {
                $userEmails = (array) $requestdata['customer_email'];
                $userIds = User::whereIn('email', $userEmails)->pluck('id')->all();
                $allJobs->whereIn('user_id', $userIds);
            }
            if (isset($requestdata['translator_email']) && count($requestdata['translator_email'])) {
                $translatorEmails = (array) $requestdata['translator_email'];
                $userIds = User::whereIn('email', $translatorEmails)->pluck('id')->all();
                $translatorJobIds = DB::table('translator_job_rel')->whereNull('cancel_at')->whereIn('user_id', $userIds)->pluck('job_id')->all();
                $allJobs->whereIn('id', $translatorJobIds);
            }
    ...
    public function alerts(Request $request)
    {
        ...
        $sesJobs->each(function ($job) use (&$jobId) {
            $jobId->push($job->id);
        });
        ...
        if ($cuser && $cuser->is('superadmin')) {
            $allJobs = Job::whereIn('id', $jobId);
    
            if ($request->filled('lang')) {
                $allJobs->whereIn('from_language_id', $request->input('lang'))->where('ignore', 0);
            }
            if ($request->filled('status')) {
                $allJobs->whereIn('status', $request->input('status'))->where('ignore', 0);
            }
            if ($request->filled('customer_email')) {
                $user = User::where('email', $request->input('customer_email'))->first();
                if ($user) {
                    $allJobs->where('user_id', $user->id)->where('ignore', 0);
                }
            }
            if ($request->filled('translator_email')) {
                $user = User::where('email', $request->input('translator_email'))->first();
                if ($user) {
                    $allJobIDs = TranslatorJobRel::where('user_id', $user->id)->pluck('job_id');
                    $allJobs->whereIn('id', $allJobIDs)->where('ignore', 0);
                }
            }
            if ($request->filled('filter_timetype')) {
                if ($request->input('filter_timetype') === 'created') {
                    if ($request->filled('from')) {
                        $allJobs->where('created_at', '>=', $request->input('from'))->where('ignore', 0);
                    }
                    if ($request->filled('to')) {
                        $to = $request->input('to') . " 23:59:00";
                        $allJobs->where('created_at', '<=', $to)->where('ignore', 0);
                    }
                    $allJobs->orderBy('created_at', 'desc');
                } elseif ($request->input('filter_timetype') === 'due') {
                    if ($request->filled('from')) {
                        $allJobs->where('due', '>=', $request->input('from'))->where('ignore', 0);
                    }
                    if ($request->filled('to')) {
                        $to = $request->input('to') . " 23:59:00";
                        $allJobs->where('due', '<=', $to)->where('ignore', 0);
                    }
                    $allJobs->orderBy('due', 'desc');
                }
            }
            if ($request->filled('job_type')) {
                $allJobs->whereIn('job_type', $request->input('job_type'))->where('ignore', 0);
            }
    
            $allJobs = $allJobs->with('fromLanguage')->where('ignore', 0)->paginate(15);
        }
    ...
    public function reopen(Request $request)
    {
        ...
        $data = [
            ...
            'will_expire_at' => TeHelper::willExpireAt($job['due'], Carbon::now()->toDateTimeString()),
        ...
        if ($job['status'] != 'timedout') {
            ...
            $job['will_expire_at'] = TeHelper::willExpireAt($job['due'], Carbon::now()->toDateTimeString());
            $job['cust_16_hour_email'] = 0;
            $job['cust_48_hour_email'] = 0;
            $job['admin_comments'] = 'This booking is a reopening of booking #' . $jobid;
            $affectedRows = Job::updateOrCreate(['id' => $jobid], $job);
        ...
        if (isset($affectedRows)) {
            ...
            return response()->json(['message' => 'Tolk cancelled!']);
        } else {
